...
export default function PharmacyCatalogPage() {
  ...
  const addToCart = (medicine) => {
    ...
    toast.success(`${medicine.name} ${t('pharmacy.addedToCart')}`)
  }
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-bold">{t('pharmacy.catalog')}</h1>
            <p className="text-gray-500 mt-1">{t('pharmacy.catalogSearch')}</p>
          </div>
        </div>
      ...
      <div className="flex gap-4">
        <div className="flex-1">
          <Input placeholder={t('pharmacy.productSearch')} value={search} onChange={(e) => setSearch(e.target.value)} icon={<Search className="h-4 w-4" />} />
        </div>
        <select
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {loading ? (
          ...
          <div className="col-span-full text-center py-12 text-gray-500">{t('pharmacy.notFound')}</div>
        ) : medicines.map(m => (
            ...
            <CardContent className="p-4">
              <div className="flex items-start justify-between mb-2">
                <div>
                  ...
                  <p className="text-xs text-gray-500">{t('medicine.barcode')}: {m.barcode}</p>
                </div>
                <Badge variant={m.quantity > 0 ? 'success' : 'danger'}>
                  {m.quantity > 0 ? `${m.quantity} ${t('pharmacy.pcs')}` : t('pharmacy.outOfStock')}
                </Badge>
              </div>
              <div className="flex items-center justify-between mt-3">
                ...
                <div className="flex items-center gap-2">
                  {m.quantity > 0 ? (
                    ...
                    <Button size="sm" disabled>{t('pharmacy.outOfStock')}</Button>
                  )}
                </div>
              </div>
            </CardContent>
