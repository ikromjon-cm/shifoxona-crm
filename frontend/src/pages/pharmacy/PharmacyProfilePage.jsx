...
export default function PharmacyProfilePage() {
  ...
  if (!form) {
    return <div className="text-center py-12 text-gray-500">Profil topilmadi</div>
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        ...
        <h1 className="text-2xl font-bold">Dorixona profili</h1>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-6">
          <Card>
            ...
            <CardContent className="space-y-4">
              <Input label="Dorixona nomi" value={form.name} onChange={(e) => update('name', e.target.value)} />
              <Input label="STIR / Litsenziya" value={form.stir_or_license || ''} onChange={(e) => update('stir_or_license', e.target.value)} />
              <Input label="Mas'ul shaxs" value={form.responsible_person || ''} onChange={(e) => update('responsible_person', e.target.value)} />
              <Input label="Telefon" value={form.phone || ''} onChange={(e) => update('phone', e.target.value)} />
            </CardContent>
          </Card>

          <Card>
            <CardHeader><CardTitle>Manzil</CardTitle></CardHeader>
            <CardContent className="space-y-4">
        ...
        <div className="space-y-6">
          <Card>
            <CardHeader><CardTitle>Joylashuv</CardTitle></CardHeader>
            <CardContent>
              ...
              <div className="grid grid-cols-2 gap-2 text-sm">
                <Input label="Kenglik" value={form.latitude || ''} onChange={(e) => update('latitude', e.target.value ? parseFloat(e.target.value) : null)} />
                <Input label="Uzunlik" value={form.longitude || ''} onChange={(e) => update('longitude', e.target.value ? parseFloat(e.target.value) : null)} />
              </div>
              {form.latitude && form.longitude && (
                ...
                >
                  <MapPin className="h-4 w-4" /> Google Maps da ochish
                </a>
              )}
            </CardContent>
          </Card>
